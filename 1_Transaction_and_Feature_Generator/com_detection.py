...
def compute_first_density(graph, communities):
    ...
    for com, sub_g in communities.items():
        ...
        for node in sub_g.nodes():
            graph.nodes[node]['first_density'] = stat
    return graph
...
def compute_second_density(graph, communities):
    for com, sub_g in communities.items():
        ...
        for node in sub_g.nodes():
            graph.nodes[node]['second_density'] = graph.nodes[node]['first_density'] / num_nodes
    return graph
    # full_density = nx.density(graph)
    # for com, sub_g in communities.items():
    #     num_nodes = sub_g.number_of_nodes()
    ...
    #     for node in sub_g.nodes():
    #         graph.nodes[node]['second_density'] = stat
    # return graph
...
def compute_third_density(graph, communities, null_samples):
    ...
    for _, sub_g in communities.items():
        ...
        for node in sub_g.nodes():
            graph.nodes[node]['third_density'] = score
    return graph

def small_community_feature(graph, communities, criterion):
    for node in graph.nodes():
        graph.nodes[node]['small_community'] = 0
    for _, sub_g in communities.items():
        num_nodes = sub_g.number_of_nodes()
        if num_nodes <= criterion:
            for node in sub_g.nodes():
                graph.nodes[node]['small_community'] = 1
    return graph
...
def compute_first_strength(graph, communities):
    ...
    for _, sub_g in communities.items():
        ...
        for node in sub_g.nodes():
            graph.nodes[node]['first_strength'] = strength
    return graph
...
def compute_second_strength(graph, communities):
    for _, sub_g in communities.items():
        num_nodes = sub_g.number_of_nodes()
        for node in sub_g.nodes():
            graph.nodes[node]['second_strength'] = graph.nodes[node]['first_strength'] / num_nodes
    return graph
