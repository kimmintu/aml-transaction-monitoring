...
def basic_features(graph, num_samples=10000):
    ...
    for node in graph.nodes():
        ...
        graph.nodes[node]['gaw_score'] = gaw_scores[0]
        graph.nodes[node]['gaw10_score'] = gaw_scores[1]
        graph.nodes[node]['gaw20_score'] = gaw_scores[2]
        # standard degree
        standard_degree = (len(edge_weights) - degree_avg) / degree_std
        graph.nodes[node]['degree_std'] = standard_degree
    return graph
...
# all_features = set()
# for node in graph.nodes():
#     node_features = set(dict(graph.nodes[node]).keys())
