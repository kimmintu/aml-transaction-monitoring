    ...
    for i in range(4):
        ...
        for j in range(5):
            try:
                ...
                print('Len obs_stat=', len(obs_stat), ', i=', i, ', j=', j)
                print(obs_stat)
                print(error)
    ...
    for comm_idx, community in enumerate(communities):
        ...
        for strength_name, strength_score in zip(strength_names, strength_scores):
            for node, score in strength_score.items():
                assert graph.nodes[node].get(strength_name) is None
                graph.nodes[node][strength_name] = score
        ...
        for idx in range(13):
            ...
            for score_idx in [1, 2]:
                for node, score in motif_score[score_idx-1].items():
                    assert graph.nodes[node].get('motif_{}_{}'.format(motif_id, score_idx)) is None
                    graph.nodes[node]['motif_{}_{}'.format(motif_id, score_idx)] = score
    ...
    for comm_idx, community in enumerate(communities_aug):
        ...
        for matrix_idx, matrix_name in enumerate(matrix_names):
            for node in community.nodes():
                graph.nodes[node]['NetEMD_{}_1'.format(matrix_name)] = matrix_scores[matrix_idx][0][node]
                graph.nodes[node]['NetEMD_{}_2'.format(matrix_name)] = matrix_scores[matrix_idx][1][node]
    return graph
